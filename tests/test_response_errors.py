...
from fastapi_testing import create_test_server, InvalidResponseTypeError
...
class TestAsyncTestResponseErrors:
    ...
    async def test_http_response_websocket_method_error(self):
        ...
        async with create_test_server() as server:
            ...
            with pytest.raises(InvalidResponseTypeError,
                               match="This response is not a WebSocket connection"):
                response.websocket()
    ...
    async def test_websocket_response_json_error(self):
        ...
        async with create_test_server() as server:
            ...
            try:
                with pytest.raises(InvalidResponseTypeError,
                                   match="Cannot get JSON directly from WebSocket response"):
    ...
    async def test_websocket_response_text_error(self):
        ...
        async with create_test_server() as server:
            ...
            try:
                with pytest.raises(InvalidResponseTypeError,
                                   match="Cannot get text directly from WebSocket response"):
    ...
    async def test_websocket_response_status_code_error(self):
        ...
        async with create_test_server() as server:
            ...
            try:
                with pytest.raises(InvalidResponseTypeError,
                                   match="WebSocket connections don't have status codes"):
